...
def js_check(path) -> dict:
    ...
    Returns:
        {'intact': bool, 'what': 'SyntaxError: …' или '', 'where': 'файл:строка',
         'chunk': строка исходника в месте поломки, 'imported_by': [адреса]}:
         у целого файла `what`/`where`/`chunk` пустые; `imported_by` — статический
