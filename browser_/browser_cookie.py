...
## Рядом живёт второй отбор cookie, и это не случайность

`http_heartbeat_cookies(storage_state, base)` делает то же самое для своего
потребителя — держателя чужой сессии без браузера (`http_heartbeat.md`), — но
отбирает **мягче**: по домену второго уровня, без проверки пути, `secure` и срока,
и отдаёт словарь для `httpx`, а не строку заголовка.

Разница не косметическая, и объединять их одним движением нельзя. Здесь правила
браузера соблюдены строго (точка в начале домена — «и поддомены», иначе точное
совпадение хоста), а там `site.com` намеренно совпадает с `www.site.com`: сайт
кладёт cookie то на один хост, то на другой, и строгий отбор оставил бы живой
heartbeat без нужной cookie. Пока у обеих свои потребители, обе и живут; сводить
их — отдельная правка с проверкой на живом сайте, а не побочный эффект переноса.
"""
