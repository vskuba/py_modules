...
def web_drive_eval(src, script, wait_ms=5000, storage=(), shot='',
    ...
    Returns:
        ...
        страницы — ещё 'where' ('url:строка') и 'chunk' (эта строка; у битого
...
def _record(answer: dict, console: list) -> None:
    ...
    Падение страницы и ошибки журнала получают ещё и `where`/`chunk`: у
    неразобранного модуля стек пуст (модуль не исполнился вовсе), и без url+line
    с куском исходника поломку ищут наугад по всей странице.
    """
