...
def web_grab(urls, to='.', *, ua=WEB_GRAB_UA, path='') -> list:
    ...
    Returns:
        список {'адрес', 'файл', 'тип', 'байт', 'почему'} на каждый скачанный
        адрес: 'тип' — угадан по голове (jpg/png/webp/json/…), 'почему' пусто,
        когда пришёл настоящий файл; страница вместо файла — тот же список, но
        с 'файл': '' и 'почему': 'страница вместо файла' — молча сохранять её
        под расширением картинки нельзя.

    ⚠ Сверяется голова с расширением уже скачанного (`file_check`): если сайт
    отдал JSON-ошибку под видом картинки — это будет видно по 'почему', а не
    молчаливым «цел».
    """
