...
def _run_one(wf, scene, out, base, seed):
    ...
    for o in done.get('outputs', {}).values():
        for img in o.get('images', []):
            ...
            b.raise_for_status()
            rows.append(_save(b.content, out, scene, seed))
    return rows


def _save(data, out, scene, seed):
    """EXIF долой, имя — хэш содержимого, строка манифеста готова."""
    d = Path(out)
    ...
    name = hashlib.sha256(tmp.read_bytes()).hexdigest()[:32] + '.jpg'
    tmp.rename(d / name)
