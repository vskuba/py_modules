...
def comfy_gen_swap(photos, workflow, out, *, base, persona='', mode='face',
    ...
    for p in map(Path, photos):
        with tempfile.TemporaryDirectory() as td:
            ...
            if ref.get('own'):
                # Эталоном стало ядро САМОГО лица — кожи рядом не нашлось.
                # Тянуть тон к самому себе бессмысленно: сдвиг нулевой, а
                # полная подгонка (`fit_full`) на нулевом сдвиге красит пятнами.
                target = probe
                fit_full = 0.0
