...
def json_from_string(string: str, raise_on_error: bool = False) -> dict:
    """Вынуть JSON-объект из ответа LLM, не рассчитывая на его чистоту.

    Модель предваряет объект болтовнёй («Готово, бла-бла. {…}»), заворачивает его
    в ```json и дописывает текст после закрывающей скобки. Функция режет всё до
    первой `{`, а хвост отбрасывает по балансу скобок.

    Args:
        string: текст ответа; готовый `dict` возвращается как есть.
        raise_on_error: поднимать исключение вместо тихого `{}`.

    Returns:
        dict: разобранный объект; при неудаче — пустой словарь.

    ⚠ Годится **только объект**. Массив верхнего уровня и примитив считаются
    ошибкой и дают `{}` — молча, если не поднят `raise_on_error`.
    """
