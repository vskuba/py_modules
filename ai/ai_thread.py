...
async def ai_thread_framework_run(ai_frameworks: list[AbstractAiFramework]):
    """Вечный цикл-диспетчер: разбирает очередь заявок к агентам и раздаёт их по слотам.

    Заявки (`AiFrameworkModel`) берутся из очереди `ai_framework_model`, движок
    подбирается по имени класса. У агента свой предел одновременных запусков
    (`llm_parallel_per_agent_max`): пока слотов нет, заявка возвращается в очередь,
    а свободному слоту достаётся LLM, не занятая соседними.

    Args:
        ai_frameworks: движки сценариев проекта — среди них ищется нужный.
