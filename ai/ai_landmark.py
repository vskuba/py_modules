...
# Зоны лица в его СОБСТВЕННОЙ системе координат: начало — середина глаз, ось x
# вдоль линии глаз в сторону видимой половины, ось y перпендикулярно вниз, к
# рту. Числа: (x в долях межглазного расстояния, y в долях «глаза→рот», радиус
# окна в долях межглазного). Долями, а не пикселями, — чтобы зона не зависела
# ни от размера лица, ни от наклона головы.
# ⚠ Подбородка здесь НЕТ намеренно. Вертикальная мерка — «глаза→рот», а она
# сжимается, когда голова поднята: на селфи с задранным подбородком замер даёт
# 0.96 межглазного против 1.13 у прямого кадра, и зона уезжает на челюсть.
# Ниже рта пяти точек нет, подпереть догадку нечем — проверено рисованием на
# двух лицах. Честнее четыре зоны, чем пять, из которых одна врёт.
AI_LANDMARK_ZONES = {
    'cheek': (0.45, 0.55, 0.22),        # чистая кожа: ни бровей, ни губ, ни ноздри
    'forehead': (0.00, -0.75, 0.26),
    'nasolabial': (0.28, 0.72, 0.13),   # складка от крыла носа к углу рта
    'eye_corner': (1.00, 0.10, 0.15),   # «гусиные лапки» СНАРУЖИ от глаза
}


def ai_landmark_zones(kps, bbox=None):
    """Окна анатомических зон лица: щека, лоб, носогубка, угол глаза.

    Отвечает на вопрос «где на этом лице мерить кожу» — и отвечает так, чтобы
    ответ пережил наклон головы. Возвращает `{зона: [x0, y0, x1, y1]}` в
    абсолютных координатах кадра; ключи — `AI_LANDMARK_ZONES`.

    Чистая геометрия по пяти точкам: зовущий, у которого `kps` уже есть
    (`ai_face_centroid`, `ai_landmark`, `ai_face_crop`), вторым проходом
    детектора не платит. `bbox` не нужен для счёта и принимается только ради
    единообразия с `ai_landmark_pose`/`ai_landmark_ellipse`.

    ⚠⚠ **Доли от КОРОБКИ лица для этого не годятся, хотя так и тянет.**
    Коробка осей не поворачивает, и на наклонённой голове точка «0.25 ширины,
    0.62 высоты» уезжает с щеки на глаз — в замер фактуры идут ресницы, в
    эталон цвета белок. Перекос выходит систематическим, а не шумом: селфи
    почти всегда с наклоном, а генерации почти всегда прямые, и сравнивается
    ресница с щекой. Здесь оси строятся по линии глаз, поэтому наклон зона
    отслеживает сама.

    ⚠ Сторона берётся ВИДИМАЯ — та, чей глаз дальше от носа. В профиль дальняя
    щека уходит за край лица, и окно на ней поймало бы фон.

    ⚠ Пять точек в глубоком профиле сближаются, и межглазное расстояние там
    вырождается — окна получаются мелкие и ненадёжные. Признак: `yaw` из
    `ai_landmark_pose` больше 0.45.
    """
    (lx, ly), (rx, ry) = [float(v) for v in kps[0][:2]], [float(v) for v in kps[1][:2]]
    nx, ny = (float(v) for v in kps[2][:2])
    lm, rm = [float(v) for v in kps[3][:2]], [float(v) for v in kps[4][:2]]
    ex, ey = (lx + rx) / 2, (ly + ry) / 2
    mx, my = (lm[0] + rm[0]) / 2, (lm[1] + rm[1]) / 2
    span = max(1.0, ((rx - lx) ** 2 + (ry - ly) ** 2) ** 0.5)
    # ось x — вдоль линии глаз, НОСОМ НАРУЖУ: к тому глазу, что дальше от носа
    sign = 1.0 if abs(lx - nx) >= abs(rx - nx) else -1.0
    ux, uy = sign * (lx - rx) / span, sign * (ly - ry) / span
    # ось y — перпендикуляр к ней, развёрнутый в сторону рта
    px, py = -uy, ux
    if px * (mx - ex) + py * (my - ey) < 0:
        px, py = uy, -ux
    drop = max(1.0, ((mx - ex) ** 2 + (my - ey) ** 2) ** 0.5)
    out = {}
    for name, (ax, ay, ar) in AI_LANDMARK_ZONES.items():
        cx = ex + ux * ax * span + px * ay * drop
        cy = ey + uy * ax * span + py * ay * drop
        r = max(4, int(ar * span))
        out[name] = [max(0, int(cx - r)), max(0, int(cy - r)),
                     int(cx + r), int(cy + r)]
    return out
