...
def deploy_wait_landed(commit: str, probe: str) -> dict:
    ...
    Args:
        ...
        probe: команда, печатающая **признак** выложенного кода. Чем именно
            проверять — зависит от стенда, и одного способа на все нет:

                ssh хост 'cd ~/app && git rev-parse HEAD'   # выкладка клонирует
                ssh хост 'cat ~/app/.commit'                # пишет метку файлом
                curl -s https://…/version                   # отдаёт версию сам

            ⚠ `git rev-parse` — не универсальный ответ: у половины стендов
            выкладка кладёт **файлы**, и `.git` там нет вовсе. Проверено на двух
            соседних проектах: у одного репозиторий есть, у другого нет.
