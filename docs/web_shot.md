...
себя: входит учёткой из `.env` **в момент вызова**, вынимает из страницы её
модуль и аргумент его инициализации без угадывания кавычек шаблона, ставит
разметку в DOM, подключает модуль — и даёт скрипту `wait(fn, ms, base)`:
вернуть значение, когда `fn()` ОТЛИЧИТСЯ от `base` (или станет истинным без
`base`). Скрипту остаётся само дело: заполнить поле, нажать, вернуть число.
