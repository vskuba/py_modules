...
async def qdrant_search(
    ...
    """Гибридный поиск по коллекции — близость по смыслу вместе с совпадением слов.

    Запрос кодируется двумя моделями сразу: плотной многоязычной и разрежённой
    BM25; имена векторов берутся из схемы самой коллекции, а не задаются здесь.

    Args:
        query_text: текст запроса.
        collection_name: коллекция; несуществующая даёт пустой список, а не ошибку.
        metadata: описание полей коллекции — по нему решается, какое условие
            обязательно (`must`), а какое лишь поднимает точку в выдаче (`should`).
        metadata_filter: значения условий; `None` и поля, которых нет в `metadata`,
            пропускаются.
        limit: сколько точек вернуть.

    Returns:
        list[models.ScoredPoint]: найденное с оценкой; пусто — коллекции нет.

    ⚠ Обязательными становятся только поля таблицы `fact`, оканчивающиеся на `_id`,
    а также `id`, `document_id` и `tags`. Остальное уходит в `should` — то есть
    **не сужает** выдачу, а лишь меняет порядок.

    ⚠ Тянет `fastembed` и веса моделей, а `HF_HUB_OFFLINE=1` выставляется на импорте
    модуля: если весов нет в кэше, вызов упадёт и скачивать их не пойдёт.
    """
    logger_info(
        f"🔍 Qdrant search: "
        f"collection_name='{collection_name}', "
